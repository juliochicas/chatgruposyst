...
export const remove = async (
  ...
  try {
    ...
    if (count > 0) return res.status(200).json({ message: "¡No fue posible eliminar! Verifique si este prompt está siendo usado en las conexiones WhatsApp." });
    ...
    return res.status(500).json({ message: "¡No fue posible eliminar! Verifique si este prompt está siendo usado." });
  }
};
